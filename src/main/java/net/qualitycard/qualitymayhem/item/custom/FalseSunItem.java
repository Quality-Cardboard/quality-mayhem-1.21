...
public class FalseSunItem extends ShieldItem {
    ...
    public static void createCircle(World world, Vec3d center, int radius, int numParticles) {
        double angleIncrement = 2 * Math.PI / numParticles;
        for (int i = 0; i < numParticles; i++) {
            double angle = i * angleIncrement;
            double x = center.getX() + radius * Math.cos(angle);
            double z = center.getZ() + radius * Math.sin(angle);
            world.addParticle(ModParticles.LIGHT_PARTICLE, x, center.getY(), z, 0, 0, 0);
        }
    }
        ...
        if (chargeTime > 5) {
            ...
            for (Entity selectedEntity : entitiesList) {
                if (selectedEntity != user) {
                    selectedEntity.damage(ModDamageTypes.of(world, ModDamageTypes.LIGHT_DAMAGE_TYPE), chargeTime / 5);
                    createCircle(world, user.getPos(), 4, 40);
                }
            }
        }
    }
